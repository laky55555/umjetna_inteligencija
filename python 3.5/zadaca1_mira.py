...
class Fig52Game(Game):
    ...
    def actions(self, state):
        return list(self.succs.get(state, {}).keys())  #keys()->Return a copy of the dictionary’s list of keys....get(..)->Return the value for key if key is in the dictionary, else default
...
class TicTacToe(Game):
    ...
    def __init__(self, h=3, v=3, k=3):
        update(self, h=h, v=v, k=k)   #????update
        moves = [(x, y) for x in range(1, h+1)
                 for y in range(1, v+1)]
        self.initial = Struct(to_move='X', utility=0, board={}, moves=moves)  #???Struct
